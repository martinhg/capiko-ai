...
- **More SDD machinery (TODO).** The OpenSpec file store is in place (config /
  ...
  - ~~**Skill registry resolution**~~ — **Done.** A native Go engine
    (`internal/skillregistry`) backs `capiko-ai skill-registry`, which scans
    `~/.copilot/skills` (user) and `<cwd>/.copilot/skills` (project) and prints the
    installed skills indexed by trigger/description, scope, and absolute `SKILL.md`
    path — always fresh, not a cached file, mirroring the `sdd-status` engine. The
    `skill-registry` catalog skill teaches delegators to run it and inject the exact
    paths into sub-agents, and the SDD orchestrator block carries a "Skill resolution"
    rule that wires the same step before every delegation. (capiko still uses
    `.atl/skill-registry.md` for its own dogfooding, generated by gentle-ai.)
